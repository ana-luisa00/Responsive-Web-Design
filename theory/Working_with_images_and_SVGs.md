...
Raster formats: images that are pixel based with the data tracking the colour value in each pixel
- do no upscale well

SVG (Scalable Vector Graphic): tracks data based on paths and equations to plot points, lines and curves
- can be scaled to any size without impacting quality
- stores data in XML which means you can use them directly in your code as raw HTML
```
...
```
- svg: container for the drawing, it sets up the space where all the shapes appear
- circle: used to make circles (duh)
- path: creates lines

## When to use an SVG?
- icons
- webpage logos
- anything you need to scale perfectly
